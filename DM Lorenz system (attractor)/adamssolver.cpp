...
std::vector<double> *AdamsSolver::solve(double time) {
    ...
    while (currentTime < time - dt) {
        ...
        double x_corr = xk[n] + (dt / 24) * (9 * x(x_pred, yk[n], zk[n]) + 19 * x(xk[n], yk[n], zk[n]) - 5 * x(xk[n-1], yk[n-1], zk[n-1]) + x(xk[n-2], yk[n-2], zk[n-2]));
        ...
        double y_corr = yk[n] + (dt / 24) * (9 * y(xk[n], y_pred, zk[n]) + 19 * y(xk[n], yk[n], zk[n]) - 5 * y(xk[n-1], yk[n-1], zk[n-1]) + y(xk[n-2], yk[n-2], zk[n-2]));
        ...
        double z_corr = zk[n] + (dt / 24) * (9 * z(xk[n], yk[n], z_pred) + 19 * z(xk[n], yk[n], zk[n]) - 5 * z(xk[n-1], yk[n-1], zk[n-1]) + z(xk[n-2], yk[n-2], zk[n-2]));
